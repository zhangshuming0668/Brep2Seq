...
from .macro import EOS_IDX, EXT_IDX
...
def _get_padding_mask(commands, seq_dim=0, extended=False):
    with torch.no_grad():
        padding_mask_ = (commands == EOS_IDX).cumsum(dim=seq_dim) == 0
        padding_mask_ = padding_mask_.float()

        if extended:
            # padding_mask doesn't include the final EOS, extend by 1 position to include it in the loss
            S = commands.size(seq_dim)
            torch.narrow(padding_mask_, seq_dim, 3, S-3).add_(torch.narrow(padding_mask_, seq_dim, 0, S-3)).clamp_(max=1)

        if seq_dim == 0:
            return padding_mask_.unsqueeze(-1)
        return padding_mask_
